...
def test_users_pagination_total(app_url, users):
    response = requests.get(f"{app_url}/api/users?page=2&size=5")
...
def test_users_pagination_page(app_url):

    response = requests.get(f"{app_url}/api/users?page=2&size=5")
    assert response.json()["page"] == page


def test_users_pagination_size(app_url):
    response = requests.get(f"{app_url}/api/users?page=2&size=5")
    assert response.json()["size"] == len(response.json()["items"])


def test_users_pagination_pages(app_url, users):
    response = requests.get(f"{app_url}/api/users??page=2&size=5")
    total = total_users(users['items'])
